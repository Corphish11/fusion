...
class BaseTracker:
    ...
    def track_videofile(self, videofilepath, optional_box=None):
      ...
      25
        ...
        result_path = 'test_res_pos4.json'

        with open(result_path, 'r') as f:
            different_view_results = json.load(f)
        # this_view_results = different_view_results[str('4')]
        this_view_results = different_view_results
        frame_list = []
        ball_list = []
        for index in range(0, len(this_view_results)):
            frame_l = this_view_results[index][0]
            # ball = this_view_results[index][0]
        ...
        while True:
            ...
            cv.putText(frame_disp, 'Press q to quit', (20, 80), cv.FONT_HERSHEY_COMPLEX_SMALL, 1,
                      font_color, 1)
            cv.putText(frame_disp, str(my_i), (20, 100), cv.FONT_HERSHEY_COMPLEX_SMALL, 1,
                      font_color, 1)
            ...
            if key == ord('q'):
                break
            elif my_i in frame_list:
                # print(frame_list)
                # print(frame_list.index(my_i))
                if len(center_y_list) > 0:
                    # ball_id = ball_list[frame_list.index(my_i)-1]
                    # print(ball_id)
                    # detect land point
                    # to do
                    for center_index in range(0, len(center_y_list)-1):
                        if center_index > 1:
                            if center_y_list[center_index-1] > center_y_list[center_index]:
                                
                                # track_dict.update({ball_id:[my_i_list[center_index-1],out_list[center_index-1]]})
                                # print(track_dict)
                                break
                ...
                # plt.plot(signal.argrelextrema(center_y_list, np.greater)[0], center_y_list[signal.argrelextrema(center_y_list, np.greater)], '+')
                # plt.plot(my_i_list,  center_y_list, color="red", linewidth=1)

                # for n_i in range(0, len(I)-1):
                    # plt.plot(I[n_i], center_y_list[n_i], 'o')

                for this_index in range(0, len(this_view_results)-1):
                    if my_i == this_view_results[this_index][0]:
                        x, y, w, h = this_view_results[this_index][1]
                        break
                # plt.show()
                # for ball in this_view_results:
                #     if ball[1] == my_i:
                #         xmin = ball[2]
                #         ymin = ball[3]
                #         xmax = ball[4]
                ...
                my_i_list = []
                out_list = []
                # poses = this_view_results[str(my_i)]
                # for pos in poses:
                ...
                init_state = [x-0.5*w, y-0.5*h, 25, 25]
                # init_state = [int(xmin), int(ymin), int(xmax)-int(xmin), int(ymax)-int(ymin)]
