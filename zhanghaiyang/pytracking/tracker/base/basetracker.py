...
class BaseTracker:
    ...
    def track_videofile(self, videofilepath, optional_box=None):
        ...
        with open("test_res_pos4_net2_15.json", 'r') as f:
            view_results = json.load(f)

        my_i = 1
        ball_id = 0
        need_list = []
        for view_results_index in range(0, len(view_results)):
            need_list.append(view_results[view_results_index][0])
        track_result = {}
        while True:
            ret, frame = cap.read()
            my_i += 1
            if frame is None:
                with open("track_result_ball_net2.json", 'w') as f:
            ...
            #     pass
            # else:
            #     track_result.update({my_i: state})
            # if (state[0] > 395 & state[0] < 740) & (state[1] < 295):
            #     track_result.update({ball_id: [my_i, state]})

            # net1
            # if ball_id > 0:
            #     if (state[0] > 497) & (state[1] < 375):
            #         track_result[ball_id].append([my_i, state])

            # net2
            if ball_id > 0:
                if (state[0] < 1374) & (state[1] < 489):
                    track_result[ball_id].append([my_i, state])
            ...
            if key == ord('q'):
                ...
                init_state = [x, y, w, h]
                self.initialize(frame, {'init_bbox': init_state})
            elif my_i in need_list:
                ball_id += 1
                x, y, w, h = view_results[need_list.index(my_i)][1]
                # init_state = [int(x-12.5), int(y-12.5), int(28), int(28)]
                init_state = [int(x), int(y), int(w), int(h)]
                track_result.update({ball_id: [[my_i, init_state]]})
                self.initialize(frame, {'init_bbox': init_state})
